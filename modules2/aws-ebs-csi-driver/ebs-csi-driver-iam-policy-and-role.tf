    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            join(":", [split("https://", data.aws_eks_cluster.springboot.identity.0.oidc.0.issuer)[1], "sub"]) : "system:serviceaccount:${var.aws-ebs-csi-driver-ns}:${var.aws-ebs-csi-driver-sa}"
          }
        }
      },
    ]
